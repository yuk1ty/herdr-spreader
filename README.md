...
- **Nested pane splits** — split panes `right`, `down`, or `auto` with an optional `ratio`, chained from the previous pane, so you can build arbitrarily deep layouts. `auto` inspects the previous pane and splits right on a wide pane or down on a tall one.
...
| `split` | `right` \| `down` \| `auto` | Direction to split from the previous pane. `auto` uses the previous pane's aspect: right when it is wide, down when it is tall. Ignored for a tab's first pane. Default: `right`. |
